...
interface ModelProps {
  className?: string;
  models: any; // Can accept a single model or an array of models.
  style?: CSSProperties;
  showDelay?: number;
  show?: boolean;
  onLoad?: () => void; // Callback for when the model loads.
  cameraPosition?: { x: number; y: number; z: number }; // More specific type.
  alt?: string;
}
...
export const Model = ({
  ...
  const shadowGroup = useRef<any>();
  const renderTarget = useRef<any>();
  const renderTargetBlur = useRef<any>();
  const shadowCamera = useRef<any>();
  const depthMaterial = useRef<any>();
  const horizontalBlurMaterial = useRef<any>();
  const verticalBlurMaterial = useRef<any>();
  const plane = useRef<any>();
  const lights = useRef<any>();
  const blurPlane = useRef<any>();
  const fillPlane = useRef<any>();
  ...
  useEffect(() => {
    if (container.current) {
      ...
      depthMaterial.current.onBeforeCompile = (shader: any) => {
  ...
  return (
    ...
    >
      ...
      {models.map((model: { position: any }, index: number) => (
        <Device
...
interface DeviceProps {
  renderer: RefObject<WebGLRenderer>; // Using RefObject for useRef type
  model: any;
  modelGroup: RefObject<Group>; // Using RefObject for the model group reference
  renderFrame: any;
  index: number;
  showDelay: number;
  setLoaded: (loaded: boolean) => void; // Assuming setLoaded is a function
  onLoad?: () => void;
  show?: boolean;
}
...
type LoadDeviceState = {
  start: () => Promise<{
    loadFullResTexture: undefined;
    playAnimation: (() => void) | undefined;
  }>;
};
...
const Device = ({
  ...
  const [loadDevice, setLoadDevice] = useState<LoadDeviceState | undefined>(
    undefined
  );
  const reduceMotion = useReducedMotion();
  ...
  useEffect(() => {
    const applyScreenTexture = async (
      texture: any,
      node: { material: { color: Color; transparent: boolean; map: any } }
    ) => {
    ...
    const load = async () => {
      ...
      gltf.scene.traverse(async (node: any) => {
        ...
        if (node.name === MeshType.Screen) {
          ...
          (placeholderScreen.current as any) = node.clone();
          ...
          loadFullResTexture = async () => {
            ...
            const fullSize = await textureLoader.loadAsync(imageUrl);
  ...
  useEffect(() => {
    ...
    const onModelLoad = async () => {
      ...
      const loadFullResTextures: any = loadFullResTexture as any;
